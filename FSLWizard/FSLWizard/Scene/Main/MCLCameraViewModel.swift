...
protocol MCLCameraViewModelDelegate: AnyObject {
...
class MCLCameraViewModel: ObservableObject {
    ...
    func setupBinding(_ input: Input) {
        ...
        $prediction
            .debounce(for: .seconds(1), scheduler: predictionQueue)
            .receive(on: RunLoop.main)
            .sink(receiveValue: { [weak self] action in
                guard let self = self,
                      let action = action,
                      action.action != .unknown,
                      action.action != .lowConfidence else {
                    return
                }
                self.performSpeech(action.label)
            })
            .store(in: &cancelBag)
        
        // Set our prediction result speech publisher
        alphabetPrediction
            .throttle(for: .seconds(3), scheduler: predictionQueue, latest: true)
            .receive(on: RunLoop.main)
            .sink(receiveValue: { [weak self] alphabet in
                guard let self = self else {
                    return
                }
                self.performSpeech(alphabet)
            })
            .store(in: &cancelBag)
    }
    ...
    private func predictActionWithWindow(_ currentWindow: [MLMultiArray?]) -> ActionPrediction? {
        ...
        guard poseCount >= minimum else {
            return ActionPrediction(action: PhrasesClassifierHand.Action.unknown, confidence: .nan)
        }
...
//        return lowConfidence ? ActionPrediction(action: PhrasesClassifierHand.Action.lowConfidence,
//                                                confidence: .nan) : actionPrediction
    }
